...
import { UsersRound, MapPin, ChevronRight } from 'lucide-react';

import { Badge, Card, EmptyState, Modal, PageHeader, Spinner } from '@shared/ui';

import { useFamilies, useFamilyDetail, type FamilyListItem } from './api';
import styles from './FamiliesPage.module.css';

/** Oilalar boshqaruvi sahifasi. */
export function FamiliesPage(): JSX.Element {
  ...
  return (
    <>
      <PageHeader
        title="Oilalar"
        description={isLoading ? 'Yuklanmoqda…' : `Ro'yxatdan o'tgan ${families.length} oila`}
      />
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Foydalanuvchilar oila yaratmagan"
        />
      ) : (
                ...
                <div className={styles.membersList}>
                  {members.map((m) => (
                    <div key={m.id} className={styles.memberCard}>
                      <div className={styles.memberAvatar}>{m.avatar_emoji}</div>
                      <div className={styles.memberInfo}>
                        <div className={styles.memberName}>
                          {m.name} <span className={styles.memberAge}>· {m.age} yosh</span>
                        </div>
